...
Section WFT.
  ...
  Implicit Types (R : rel₂ X) (P : rel₁ (list X)) (φ : nat → X).
  ...
  Proof.
    induction 1 as [ | l _ (f & Hf)%FunChoice_type ].
    + now exists leaf.
    + exists (node f); assumption.
  Qed.
  ...
  Theorem af_recollection R : 
          ( af R                                        → { t | ∀φ, good R (pfx_rev φ (WFT_ht φ t)) } )
        * ( { t | ∀φ, good R (pfx_rev φ (WFT_ht φ t)) } → { t | af_secures R t } )
        * ( { t | af_secures R t }                      → af R ).
  Proof.
    repeat split.
    + apply af_brouwer.
    + intros (t & Ht); exists (WFT_lift t); now apply brouwer_af.
    + apply af_secures_af.
  Qed.
